...
export = async (client: ExtendedClient, Discord: typeof import("discord.js"), interaction: CommandInteraction) => {
    try {
        ...
        if(command.ownerOnly && client.config_main.owner !== interaction.user.id) return await interaction.reply({ embeds: [noPermissionCommand], ephemeral: true });
        ...
        if(client.config_main.owner === interaction.user.id) {
            // Log interaction to console
            console.log(`[interactionCreate] [command] ${interaction.user.tag.endsWith("#0") ? interaction.user.username : interaction.user.tag} (${interaction.user.id}): /${interaction.commandName} ${interaction.options.data.map((option: any) => option.value ? `${option.name}:${option.value}` : option.name).join(" ")}`);
        ...
        try {
            // Log interaction to console
            console.log(`[interactionCreate] [command] ${interaction.user.tag.endsWith("#0") ? interaction.user.username : interaction.user.tag} (${interaction.user.id}): /${interaction.commandName} ${interaction.options.data.map((option: any) => option.value ? `${option.name === "api-key" ? `${option.name}:[CENSORED]` : `${option.name}:${option.value}`}` : option.name).join(" ")}`);
