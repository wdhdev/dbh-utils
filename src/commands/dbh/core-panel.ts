...
const command: Command = {
    ...
    async execute(interaction: CommandInteraction & any, client: ExtendedClient, Discord: typeof import("discord.js")) {
        try {
            ...
            try {
                const res = await axios.post(`https://ptero.core.danbot.host/api/client/servers/${server}/power`, {
                    signal: type
                }, {
                    headers: {
    ...
    async autocomplete(interaction: AutocompleteInteraction, client: ExtendedClient) {
        ...
        if(option.name === "server") {
            ...
            try {
                const res = await axios.get("https://ptero.core.danbot.host/api/client", {
                    headers: {
                        "Accept": "application/json",
                        "Authorization": `Bearer ${data.core_panel_key}`,
                        "User-Agent": "DBH"
