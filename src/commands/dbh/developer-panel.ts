...
const command: Command = {
    ...
    description: "Manage a DBH developer panel server.",
    options: [
        {
            type: 1,
            name: "kill",
            description: "Kill a DBH developer panel server.",
            options: [
                {
                    type: 3,
                    name: "server",
...

        {
            type: 1,
            name: "restart",
            description: "Restart a DBH developer panel server.",
            options: [
                {
                    type: 3,
                    name: "server",
...

        {
            type: 1,
            name: "start",
            description: "Start a DBH developer panel server.",
            options: [
                {
                    type: 3,
                    name: "server",
...

        {
            type: 1,
            name: "stop",
            description: "Stop a DBH developer panel server.",
            options: [
                {
                    type: 3,
                    name: "server",
        ...
        try {
            ...
            if(!data || !data.developer_panel_key) {
                const error = new Discord.EmbedBuilder()
                    ...
                    .setDescription(`${emoji.cross} You have not set your developer panel API key! Run </set-developer-api-key:${client.commandIds.get("set-developer-api-key")}> to set it.`)
            ...
            try {
                const res = await axios.post(`https://${main.developerPanel}/api/client/servers/${server}/power`, {
                    signal: type
                }, {
                    headers: {
                        "Accept": "application/json",
                        "Authorization": `Bearer ${data.developer_panel_key}`,
                        "User-Agent": "william"
                    }
                })
    ...
    async autocomplete(interaction: AutocompleteInteraction, client: ExtendedClient) {
        ...
        if(option.name === "server") {
            ...
            const data = await API.findOne({ _id: interaction.user.id });
            if(!data || !data.developer_panel_key) return await interaction.respond(choices);

            // Get servers from Pterodactyl API
            try {
                const res = await axios.get(`https://${main.developerPanel}/api/client`, {
                    headers: {
                        "Accept": "application/json",
                        "Authorization": `Bearer ${data.developer_panel_key}`,
                        "User-Agent": "william"
